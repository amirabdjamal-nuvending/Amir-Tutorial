...
Function: Answer processing for AT commands
参数  ：无
返回值：无
]]
local function rsp()
    --Stop answering the timeout timer
    sys.timerStopAll(atimeout)
    --The answer handler is specified synchronously when the AT command is sent
    if currsp then
        currsp(currcmd, result, respdata, interdata)
    --The user-registered answer handler table found the handler
    else
