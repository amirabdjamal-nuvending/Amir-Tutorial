--- Module functions: data link activation (creation, connection, status maintenance)
-- @module link
-- @author openLuat
-- @license MIT
-- @copyright openLuat
-- @release 2017.9.20
--4G network does not manually activate pdp, register online after sending cgdcont IP_READY_IND?，
...
--[[
If it is the default hosting, it cannot be activated，
If it is manually activated pdp
There is also a default hosting after the cid_manual is activated，
So if the upper layer is going to initiate the socket after activation, it can be connected，
So report the IP_ERROR_IND directly here
The logic behind the shutter is managed by the upper echelons themselves
]]
function shut()
