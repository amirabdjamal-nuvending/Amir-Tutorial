...
app.post("/getLifterEnableList_V2", (req, res) => {
  ...
  connection.connect((error) => {
    if (error) {
      ...
      var query = "SELECT * FROM db_machine_lifter_enable";
      connection.query(query, (errors, results, fields) => {
        if (errors) {
          connection.end();
          console.log(error);
          ...
          results.forEach(item => {
            lifter_enable_list.push(item.lifter_enable);
          });
          res.send({
            status: 'OK',
...
app.post("/getLifterExist_V2", (req, res) => {
  var in_which_machine = 0;
  const row = req.body.row;
  ...
  connection.connect((error) => {
    if (error) {
      ...
      Promise.all(promises).then(() => {
        var query = "SELECT * FROM db_machine_lifter_enable WHERE machine_number = ?";
        var values = [in_which_machine];
        connection.query(query, values, (errors, results, fields) => {
          if (errors) {
            connection.end();
            console.log(error);
            ...
            res.send({
              status: 'OK',
              is_lifter_exist: results[0].lifter_enable == 1 ? true : false,
            });
          }
        });
      }).catch(error => {
...
app.post("/getAnyLifterExist_V2", (req, res) => {
  var connection = mysql.createConnection(dbConnectionParam);
  ...
  connection.connect((error) => {
    if (error) {
      ...
      var query = "SELECT * FROM db_machine_lifter_enable WHERE lifter_enable = 1";
      connection.query(query, (errors, results, fields) => {
        if (errors) {
          connection.end();
          console.log(error);
          ...
          if (results.length > 0) {
            res.send({
              status: 'OK',
              is_lifter_exist: true,
            });

          } else {
            res.send({
              status: 'OK',
              is_lifter_exist: false,
            });
          }
...
app.post("/setupLifterEnable_V2", (req, res) => {
  const lifter_enable_list = req.body.lifter_enable_list;
  ...
  connection.connect((error) => {
    if (error) {
      ...
      var promises1 = [];
      promises.push(new Promise((resolve, reject) => {
        var query = "DELETE FROM db_machine_lifter_enable";
        connection.query(query, (errors, results, fields) => {
          if (errors) {
            console.log(errors);
            reject('error');
      ...
      Promise.all(promises).then(() => {
        lifter_enable_list.forEach((item, index) => {
          promises1.push(new Promise((resolve1, reject1) => {
            var query = "INSERT INTO db_machine_lifter_enable (machine_number,lifter_enable,date_modified) VALUES (?,?,?)";
            var values = [Number(item.machine_number), Number(item.lifter_enable), new Date()];
            connection.query(query, values, (errors, results, fields) => {
              if (errors) {
                reject1(errors);
              } else {
        ...
        Promise.all(promises).then(() => {
          product_detail_list.forEach((resultsItem, resultIndex) => {
            ...
            promises1.push(new Promise((resolve1, reject1) => {
              ...
              var query = "INSERT INTO db_product (motor_number,created_date,price,new_price,product_name,motor_label,tray_number,slot_number,inventory,max_capacity,drop_sensor_enable,quarter_turn_enable,continue_order,product_image_url,second_pwp_price,third_pwp_price) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
              var values = [Number(resultsItem.motor_number), new Date(), 0, resultsItem.new_price, resultsItem.product_name, resultsItem.motor_label, resultsItem.tray_number, resultsItem.slot_number, resultsItem.inventory, resultsItem.max_capacity, resultsItem.drop_sensor_enable, resultsItem.quarter_turn_enable, resultsItem.continue_order, resultsItem.image_url, 0, 0];
...
app.post('/getMachineAllInfoShopping', (req, res) => {
  console.log('getMachineAllInfoShopping');
  var connection = mysql.createConnection(dbConnectionParam);
  ...
  connection.connect((error) => {
    if (error) {
      ...
      Promise.all(promises).then(() => {
        ...
        tray_slot_info.forEach(item => {
          var index_1 = tray_in_machine_info.findIndex(x => x.tray_number == item.tray_number);
          if (index_1 > -1) {
            item.in_machine = tray_in_machine_info[index_1].in_machine;
          }
          var index_2 = lifter_step_info.findIndex(x => x.tray_number == item.tray_number);
          if (index_2 > -1) {
            item.step_distance = lifter_step_info[index_2].step_distance;
          } else {
            item.step_distance = -1;
          }
        });
        connection.end();
        res.send({
