...
class SerialCom(multiprocessing.Process):
    ...
    def __init__(self, keyboard_input: KeyboardInput):
        ...
        self.serial_device = serial.Serial(
            port='/dev/serial/by-path/platform-fd500000.pcie-pci-0000:01:00.0-usb-0:1.3:1.0-port0', baudrate=57600,
            timeout=80)
        ...
        # Sensor parameters
        self.door_status = self.manager.Value('i', 1)  # 1 = Open, 0 = Closed
        self.main_temperature = self.manager.Value('i', 0)
        self.sub_temperature = self.manager.Value('i', -1)  # -1 = Not exist
    ...
    def write_to_VMC(self):
        if self.keyboard_input.get_value() == '' and not self.is_command_receive:
            ...
            if keyboard_value[0] == 'checkMotorStatus':
                ...
                content.extend(Constant.DROP_SENSOR_TEST_SUB_MACHINE)
                ...
                print("Send Command")
                content = bytearray()
                content.extend(Constant.COIN_DEVICE_SETUP_MODE)
                ...
                full_command = self.generate_full_command(stx=Constant.STX,
                                                          ...
                                                          content=content)
                self.serial_device.write(full_command)
            elif keyboard_value[0] == 'testLifter':
                machine = keyboard_value[1]  # main or sub
                row_number = int(keyboard_value[2])  # 0 - 9
                print("Send Command")
                content = bytearray()
                content.extend(Constant.LIFTER_TEST_COMMAND)
                content.extend(Constant.LIFTER_TEST_PARAMETER)
                if machine == 'main':
                    content.extend(Constant.LIFTER_MAIN_MACHINE)
                else:
                    content.extend(Constant.LIFTER_SUB_MACHINE)
                ...
                full_command = self.generate_full_command(stx=Constant.STX,
                                                          command=Constant.SETTING_COMMAND,
                                                          length=Constant.LIFTER_TEST_LENGTH,
                                                          content=content)
                ...
                full_amount = int(keyboard_value[1])
                content = bytearray()
                content.extend(Constant.PAYMENT_TYPE_COIN)
                content.extend(bytearray(full_amount.to_bytes(4, 'big')))
                full_command = self.generate_full_command(stx=Constant.STX,
                                                          command=Constant.MACHINE_UPDATE_BALANCE_COMMAND,
                                                          length=Constant.MACHINE_UPDATE_BALANCE_LENGTH,
                ...
                full_command = self.generate_full_command(stx=Constant.STX,
                                                          ...
                                                          content=content)
                self.serial_device.write(full_command)
            elif keyboard_value[0] == 'readQuarterSetting':
                print("Send Command")
                machine = keyboard_value[1]  # main or sub
                content = bytearray()
                content.extend(Constant.QUARTER_TURN_COMMAND)
                content.extend(Constant.QUARTER_TURN_READ_SETTING_PARAMETER)
                if machine == 'main':
                    machine_number = 3000
                else:
                    machine_number = 3001
                content.extend(bytearray(machine_number.to_bytes(2, 'big')))
                full_command = self.generate_full_command(stx=Constant.STX,
                                                          command=Constant.SETTING_COMMAND,
                                                          length=Constant.QUARTER_TURN_READ_SETTING_LENGTH,
                                                          content=content)
                self.serial_device.write(full_command)
            elif keyboard_value[0] == 'writeQuarterSetting':
                print("Send Command")
                machine = keyboard_value[1]  # main or sub
                on_or_off = keyboard_value[2]  # on or off
                content = bytearray()
                content.extend(Constant.QUARTER_TURN_COMMAND)
                content.extend(Constant.QUARTER_TURN_WRITE_SETTING_PARAMETER)
                if machine == 'main':
                    machine_number = 3000
                else:
                    machine_number = 3001
                ...
                if machine == 'main':
                    if on_or_off == 'on':
                        content.extend(Constant.QUARTER_TURN_WRITE_ENABLE_MAIN_MACHINE)
                    else:
                        content.extend(Constant.QUARTER_TURN_WRITE_DISABLE_MAIN_MACHINE)
                else:
                    if on_or_off == 'on':
                        content.extend(Constant.QUARTER_TURN_WRITE_ENABLE_SUB_MACHINE)
                    else:
                        content.extend(Constant.QUARTER_TURN_WRITE_DISABLE_SUB_MACHINE)
                full_command = self.generate_full_command(stx=Constant.STX,
                                                          command=Constant.SETTING_COMMAND,
                                                          length=Constant.QUARTER_TURN_WRITE_SETTING_LENGTH,
                                                          content=content)
                self.serial_device.write(full_command)
                ...
                full_command = self.generate_full_command(stx=Constant.STX,
                                                          ...
                                                          content=content)
                self.serial_device.write(full_command)
            elif keyboard_value[0] == 'readTemperatureControllerStatus':
                print("Send Command")
                machine = keyboard_value[1]  # main or sub
                content = bytearray()
                content.extend(Constant.TEMPERATURE_CONTROLLER_STATUS_COMMAND)
                content.extend(
                    Constant.TEMPERATURE_CONTROLLER_STATUS_PARAMETER)
                if machine == 'main':
                    content.extend(Constant.MAIN_MACHINE)
                else:
                    content.extend(Constant.SUB_MACHINE)
    ...
    def decode_serial(self, message: []):
        ...
        if actual_byte_array == Constant.VMC_ACK:
            ...
            self.write_to_VMC()
            # if self.keyboard_input.get_value().split(' ')[0] != 'runSpiralMotor' and \
            #         self.keyboard_input.get_value().split(' ')[0] != 'readLifterSetting' and \
            #         self.keyboard_input.get_value().split(' ')[0] != 'checkMotorStatus' and \
            #         self.keyboard_input.get_value().split(' ')[0] != 'deductBalance':
            #     self.keyboard_input.set_value('')
        else:
            ...
            if command == Constant.MOTOR_DISPENSING_STATUS_COMMAND:
                ...
                if command_type == Constant.TRACK_INSPECTION_SETTING_COMMAND:
                    ...
                    self.set_is_reading_or_setting(is_reading_or_setting=0)
                elif command_type == Constant.TEMPERATURE_CONTROLLER_STATUS_COMMAND:
                    status = actual_byte_array[7:8]
                    if status == Constant.TEMPERATURE_CONTROLLER_STATUS_READ_SUCCESS:
                        print("Read temperature success")
                        temperature = actual_byte_array[8]
                        print(temperature)
                    else:
            ...
            while True:
                if not self.trigger_new_message:
                    ...
                    if not self.is_initial and len(self.new_message) == 0:
                        self.empty_array_counter += 1
                    else:
                        self.empty_array_counter = 0

                    if self.empty_array_counter > 3:  # Three continuous empty array
