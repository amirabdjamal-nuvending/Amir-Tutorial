...
def trackInspection():
    ...
    keyboard_input.set_value(message="trackInspection " + str(motor_result.new_motor_number))
...
def checkMotorStatus():
    ...
    keyboard_input.set_value(message="checkMotorStatus " + str(motor_result.new_motor_number))
...
def dispenseItem():
    ...
    print("dispense status")
    print(serial_com.get_dispense_status())
    if serial_com.get_dispense_status() == 1:
        dispense_result = 'OK'
    else:
        dispense_result = 'Fail'
...
def setupTemperature():
    ...
    keyboard_input.set_value(message="setTemperatureController " + machine_type + " " + temperature)
...
def writeLifterSetting():
    ...
    keyboard_input.set_value(
        message="writeLifterSetting " + machine_type + " " + lifter_enabled + " " + over_current_protection_enabled)
...
def setLifterPosition():
    ...
    keyboard_input.set_value(message="setLifterPosition " + machine_type + " " + row_number + " " + position)
    #keyboard_input.set_value(message="setLifterPosition " +"main" + " " + "6" + " " + "550")
    while serial_com.get_is_reading_or_setting() == -1:
        i = 0
...
def readLifterPosition():
    ...
    # 0 - 9 must be string
    row_number = request_body["row_number"]
    keyboard_input.set_value(message="readLifterPosition " + machine_type + " " + row_number)
...
@app.route('/ghlQRSettlement', methods=['POST'])
def ghlQRSettlement():
    return jsonify(
        status=im20_library.qrSettlement()
    )
@app.route('/ghlAmexSettlement', methods=['POST'])
def ghlAmexSettlement():
    return jsonify(
        status=im20_library.amexSettlement()
    )

@app.route('/ghlCardSettlement', methods=['POST'])
def ghlCardSettlement():
    return jsonify(
        status=im20_library.cardSettlement()
    )


@app.route('/ghlRefund', methods=['POST'])
def ghlRefund():
    request_body = request.json
    id = request_body["id"]
    refundAmount = request_body["refundAmount"]
    result = im20_library.getIdData(id)
    if not result:
        return jsonify(
        ...
        result = im20_library.refund(result[0], refundAmount)
        return jsonify(
            status=result
        )


@app.route('/ghlPayment', methods=['POST'])
def ghlPayment():
    request_body = request.json
    id = request_body["id"]
    cmd = request_body["cmd"]
    hostNum = request_body["hostNum"]
    accType = request_body["accType"]
    amount = request_body["amount"]

    if id == 0:
        result = im20_library.insertGhlPayment(accType, amount)
    else:
        result = id

    im20_library.set_is_processing(1, result, cmd, hostNum, accType, amount)
...
@app.route('/abortGhlPayment', methods=['POST'])
def abortGhlPayment():
    im20_library.abortTransaction()
